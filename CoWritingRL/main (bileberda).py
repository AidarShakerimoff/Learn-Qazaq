...
def please_try_typing ():
  n = random.randint(0,4)
  if (n == 0):
    return "Please use the Kazakh-Latin alphabet to type/write the following:"
  elif (n == 1):
    return "Using the Kazakh-Latin alphabet, type the following:"
  elif (n == 2):
    return "Please try typing the following letter/word/phrase/sentence using the Kazakh-Latin alphabet:"
  elif (n == 3):
    return "What is the Kazakh-Latin version of the following letter?"
  elif (n == 4):
    return "How would you write the following letter/word/phrase/sentence in the Kazakh-Latin alphabet?"
...
def no_mistakes ():
  ...
  if (n == 0):
    ...
    return "You sure have a way with letters!"
  elif (n == 8):
    return "You’re a letter converting machine!"
...
def count_letters(phrase):
  #kazakh_letters = a, ä, b, v, g, ğ, d, e, j, z, i, k, q, l, m, n, ñ, o, ö, p, r, s, t, u, ū, ü, f, h, ş, y, ı
  ...
  for letter in latin_phrase:
    if (letter == 'a'):
      ...
      count_kazakh_letters[30] = latin_phrase[30] + 1
 ...
 # this is a method for assessing a word to a student. Provide a word and wait him/her to rewrite it. Also count time for response
  ...
  def process(self, message):
    ...
    if time_elapsed>10:
      time_elapsed = 10
    self.state = []
    self.state.append('num of letters: ' + str(31 - np.count_nonzero(self.letters)))
    self.state.append('time: ' + str(time_elapsed))
    self.state.append('num of errors: ' + str(len(self.errors)))
    self.state.append('theme: ' + str(self.theme))
    self.state.append('gender: ' + str(self.gender))
    self.state = ' '.join(self.state)
    
    self.action = self.act(self.state, self.epsilon)

    # done = False
    changed = False
    self.logfile.write("Action selected by agent: " + self.actions_codes[self.action] + "\n")
    ...
    while not passed:
      try:
        ...
        if self.action == 0:
          if not self.errors:
            self.qValues[(self.state, self.action)] = -math.inf
            self.action = 1
            changed = True
            self.logfile.write("No errors, action was changed to: " + self.actions_codes[self.action] + "\n")
          else:
        ...
        if self.action == 2:
          if not self.errors:
            ...
            self.action = 3
            changed = True
            self.logfile.write("Action was changed to " + self.actions_codes[self.action] + "\n")
          else:
  ...
  def process2(self, message):
    ...
    next_state.append('time: ' + str(self.time_elapsed))
  ...
  #  for ele in range(0, len(self.letters)):
   #   total = total + self.letters[ele]
    ...
    self.gameIter.append((self.state, self.action, reward, next_state))
...
# iteration of episodes
 ...
 # this is a method for assessing a word to a student. Provide a word and wait him/her to rewrite it. Also count time for response
  ...
  def start_episode(self, message):
    ...
    sent = bot.send_message(message.chat.id, please_try_typing() + "\n" + 
      "\n" + "                               " + self.word)
    self.start_time = time.time()
    #self.logfile.close()
...
############# UNTIL HERE ##########################
    ...
    #print(">>>>>interaction with student begins")
    self.info[4] = np.count_nonzero(self.letters)
    sent2 = bot.send_message(message.chat.id, please_try_typing() + "\n" + 
      "\n" + "                               " + self.word)
    self.start_time = time.time()
    self.logfile.close()
